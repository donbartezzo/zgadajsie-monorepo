...
> Klucz prywatny musi trafić do `~/apps/zgadajsie/shared/.env` na serwerze (`VAPID_PRIVATE_KEY`).
...
- `shared/.env` poza katalogiem release
...
2. Rozpakuj artefakt
3. Symlink `.env` z `shared/.env` → `dist/backend/.env` i `NEW_RELEASE/.env`
...
- [ ] Stworzyć `~/apps/zgadajsie/shared/.env` z wszystkimi sekretami
...
- [ ] Stworzyć symlink `.env` w `public_nodejs/`:
  ```bash
  ln -sf ~/apps/zgadajsie/shared/.env ~/domains/api.zgadajsie.pl/public_nodejs/.env
  ```
