      ...
      <div class="grid grid-cols-2 gap-x-4 gap-y-2 pt-2 border-t border-neutral-100">
        @if (_event && _event.costPerPerson > 0) {
          <div class="space-y-0.5">
            ...
            <p class="text-sm font-semibold text-neutral-800">{{ _event.costPerPerson }}z</p>
          </div>
        }
        ...
        @if (assignedAtFormatted()) {
          <div class="space-y-0.5">
            <p class="text-[10px] uppercase tracking-wide text-neutral-400 font-medium">Zajte</p>
    ...
    switch (this.slotStatus()) {
      case 'free':
        return e && e.costPerPerson > 0
          ? `Koszt udziau: ${e.costPerPerson}z`
          : 'Wydarzenie bezpatne';
      case 'locked':
