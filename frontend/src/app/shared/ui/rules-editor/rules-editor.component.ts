...
@Component({
  ...
  template: `
    <div class="space-y-2">
      ...
      <div
        class="border border-gray-300 dark:border-slate-600 rounded-xl p-3 bg-white dark:bg-slate-700 min-h-[100px]"
      >
        @if (rules().length === 0) {
        ...
        <div class="space-y-2">
          ...
          <div class="flex items-start gap-2 group" [style.margin-left.px]="rule.indent * 20">
            <span class="text-gray-500 dark:text-gray-400 text-xs mt-2.5 min-w-[20px]">
              {{ index + 1 }}.
            </span>
            ...
            <input
              ...
              class="flex-1 rounded-lg border border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-700 px-2 py-1.5 text-sm text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-1 focus:ring-highlight"
            />
