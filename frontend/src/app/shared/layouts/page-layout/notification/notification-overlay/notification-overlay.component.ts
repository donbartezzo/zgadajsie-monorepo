...
@Component({
  ...
  template: `
    @let _state = state(); @let _cfg = config(); @let _perm = pushPermission();
    @let _requesting = requestingPush();

    <app-bottom-overlay [open]="isOpen()" (closed)="close()">
      @if (_cfg) {
        @let _isCity = _cfg.resourceType === 'city'; @let _label = _cfg.resourceLabel;
        ...
        <div class="space-y-4 max-w-lg mx-auto">
            ...
            @switch (_state) {
              ...
              @case ('OFF') {
                ...
                <p class="mt-1 text-sm text-neutral-500">
                  @if (_isCity) {
                    Włącz powiadomienia, żeby dostawać informacje o nowych wydarzeniach w mieście
                    <span class="font-semibold">{{ _label }}</span
                    >.
                  } @else {
                    Włącz powiadomienia, żeby być na bieżąco ze zmianami dotyczącymi tego
                    wydarzenia.
                  }
                </p>
              }
              ...
              @case ('ON_COMPLETE') {
                ...
                <p class="mt-1 text-sm text-neutral-500">
                  @if (_isCity) {
                    Będziesz dostawać powiadomienia o nowych wydarzeniach w mieście
                    <span class="font-semibold">{{ _label }}</span
                    >.
                  } @else {
                    Otrzymujesz powiadomienia o zmianach dotyczących tego wydarzenia.
                  }
                </p>
              }
              ...
              @case ('ON_INCOMPLETE') {
                ...
                <p class="mt-1 text-sm text-neutral-500">
                  @if (_isCity) {
                    Włącz dodatkowe powiadomienia Push, żeby szybciej dowiadywać się o nowych
                    wydarzeniach w mieście <span class="font-semibold">{{ _label }}</span
                    >.
                  } @else {
                    Włącz dodatkowe powiadomienia Push, żeby natychmiast reagować na zmiany w
                    wydarzeniu.
                  }
                </p>
              }
          ...
          <p class="text-xs text-neutral-400 text-center">
            @if (_isCity) {
              Miasto: <span class="font-semibold text-neutral-600">{{ _label }}</span>
            } @else {
              Wydarzenie: <span class="font-semibold text-neutral-600">{{ _label }}</span>
            }
          </p>
          ...
          <div class="space-y-2">
            ...
            @if (_state === 'OFF') {
              <div
                class="flex items-center gap-2 rounded-lg border border-neutral-200 bg-neutral-50 px-3 py-2"
              >
              ...
              <div
                class="flex items-start gap-2 rounded-lg border border-danger-200 bg-danger-50 px-3 py-2"
              >
              ...
              <div
                class="flex items-center justify-between gap-2 rounded-lg border border-warning-200 bg-warning-50 px-3 py-2"
              >
