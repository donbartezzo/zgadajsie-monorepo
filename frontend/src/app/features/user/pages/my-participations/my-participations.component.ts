  ...
  template: `
    <div class="p-4">
      ...
      @if (loading()) {
        ...
        <app-empty-state icon="users" title="Brak uczestnictw" message="Nie dołączyłeś jeszcze do żadnego wydarzenia."></app-empty-state>
      } @else {
        <div class="space-y-3">
              ...
              <div class="p-4 flex items-center justify-between">
                <div>
                  ...
                  <span class="text-xs mt-1 inline-block px-2 py-0.5 rounded-full"
                    [class]="p.status === 'ACCEPTED' ? 'bg-green-100 text-green-700 dark:bg-green-900 dark:text-green-300' : p.status === 'PENDING' ? 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900 dark:text-yellow-300' : 'bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-400'">
                    {{ p.status }}
                  </span>
                </div>
...
export class MyParticipationsComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.userService.getMyParticipations().subscribe({
      next: (p) => { this.participations.set(p); this.loading.set(false); },
