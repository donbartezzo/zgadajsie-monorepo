    ...
    <div class="py-6">
      @if (auth.currentUser(); as user) {
        <div class="text-center mb-6">
          ...
          <h1 class="mt-3 text-xl font-bold text-gray-900 dark:text-gray-100">{{ user.displayName }}</h1>
          <p class="text-sm text-gray-500 dark:text-gray-400">{{ user.email }}</p>
          @if (!user.isEmailVerified) {
            <div class="mt-2 rounded-lg bg-yellow-50 dark:bg-yellow-900/20 px-3 py-2 text-xs text-yellow-700 dark:text-yellow-300">
              Email nie zweryfikowany.
              <button (click)="resendActivation()" class="underline font-medium ml-1">Wyślij link ponownie</button>
            </div>
          }
        </div>
          ...
          <a routerLink="/profile/events">
            <app-card>
              <div class="p-3 flex items-center gap-2">
                ...
                <span class="text-sm font-medium text-gray-700 dark:text-gray-300">Moje wydarzenia</span>
              </div>
            </app-card>
          </a>
