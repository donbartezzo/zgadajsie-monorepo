...
export class EventCriteriaDescriptionComponent {
  ...
  readonly criteria = computed<CriteriaItem[]>(() => {
    ...
    if (venueText) {
      items.push({ icon: 'map-pin', iconColor: 'text-red-500 dark:text-red-400', text: venueText });
    }

    const dateText = this.buildDateText(e);
    if (dateText) {
      items.push({ icon: 'clock', iconColor: 'text-blue-500 dark:text-blue-400', text: dateText });
    }
  ...
  private buildParticipantsText(e: EventModel): string {
    ...
    if (min && max) {
      return (
        `W wydarzeniu może wziąć udział maksymalnie <strong>${max}</strong> ${this.getPersonWord(max)} ` +
        `i co najmniej <strong>${min}</strong>, żeby wydarzenie w ogóle mogło się odbyć.`
      );
    }

    if (max) {
      return `W wydarzeniu może wziąć udział maksymalnie <strong>${max}</strong> ${this.getPersonWord(max)}.`;
    }

    return `Do odbycia się wydarzenia potrzeba co najmniej <strong>${min}</strong> ${this.getPersonWord(min ?? 0)}.`;
  }
