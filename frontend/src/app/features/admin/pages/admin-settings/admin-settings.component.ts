    ...
    <div class="py-6">
      ...
      @if (loading()) {
          ...
          <div class="p-4 space-y-3">
            ...
            @for (s of settings(); track s.key) {
              <div class="flex items-center gap-2">
                ...
                <input [(ngModel)]="s.value" class="flex-1 rounded-xl border border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-700 px-3 py-1.5 text-sm text-gray-900 dark:text-gray-100" />
                <app-button variant="outline" size="sm" (clicked)="saveSetting(s)"><app-icon name="check" size="sm"></app-icon></app-button>
              </div>
            }
          </div>
        </app-card>
        ...
        @for (dict of dictTypes; track dict.type) {
          <app-card>
            <div class="p-4 mb-3">
              <h3 class="text-sm font-semibold text-gray-900 dark:text-gray-100 mb-2">{{ dict.label }}</h3>
              <div class="space-y-1">
                @for (item of dict.items(); track item.id) {
                  <div class="flex items-center gap-2 text-sm">
                    <span class="flex-1 text-gray-700 dark:text-gray-300">{{ item.name }}</span>
                    <app-button variant="danger" size="sm" (clicked)="deleteDict(dict.type, item.id, dict.items)"><app-icon name="trash" size="sm"></app-icon></app-button>
                  </div>
                }
              </div>
              <div class="flex gap-2 mt-2">
                <input [(ngModel)]="dict.newName" placeholder="Nowa pozycja" class="flex-1 rounded-xl border border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-700 px-3 py-1.5 text-sm text-gray-900 dark:text-gray-100" />
                <app-button variant="primary" size="sm" (clicked)="addDict(dict)"><app-icon name="plus" size="sm"></app-icon></app-button>
              </div>
            </div>
          </app-card>
        }
...
export class AdminSettingsComponent implements OnInit {
  ...
  readonly dictTypes: { type: string; label: string; items: ReturnType<typeof signal<DictionaryItem[]>>; newName: string }[] = [
  ...
  addDict(dict: typeof this.dictTypes[0]): void {
    ...
    this.adminService.createDictionary(dict.type, { name: dict.newName.trim(), slug }).subscribe({
      next: (item) => { dict.items.update(prev => [...prev, item]); dict.newName = ''; this.snackbar.success('Dodano'); },
      error: () => this.snackbar.error('Błąd'),
    });
  }
  ...
  deleteDict(type: string, id: string, items: ReturnType<typeof signal<DictionaryItem[]>>): void {
    this.adminService.deleteDictionary(type, id).subscribe({
      next: () => { items.update(prev => prev.filter(i => i.id !== id)); this.snackbar.info('Usunięto'); },
      error: () => this.snackbar.error('Błąd'),
    });
  }
}
